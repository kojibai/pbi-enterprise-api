...
export function loadStoredCredential() {
    ...
    try {
        ...
        if (typeof v.credIdB64Url !== "string")
            return null;
        if (typeof v.pubKeyPem !== "string")
            return null;
...
export async function registerPasskey() {
    ...
    if (!(att instanceof Map))
        throw new Error("bad_attestation_cbor");
    const authData = att.get("authData");
    if (!(authData instanceof Uint8Array))
        throw new Error("missing_authData");
...
function parseAuthDataForCredential(authData) {
    if (authData.length < 37)
        throw new Error("authData_too_short");
    const flags = authData[32];
    if (flags === undefined)
        throw new Error("missing_flags");
    const AT = (flags & 0x40) !== 0;
    if (!AT)
        throw new Error("attested_data_missing_AT_flag");
    let i = 37; // 32 + 1 + 4
    i += 16; // aaguid
    if (i + 2 > authData.length)
        throw new Error("authData_bad");
    const b0 = authData[i];
    ...
    i += 2;
    if (i + credIdLen > authData.length)
        throw new Error("authData_bad_credId");
    ...
    if (!(cose instanceof Map))
        throw new Error("bad_cose");
    const kty = cose.get(1);
    const crv = cose.get(-1);
    const x = cose.get(-2);
    const y = cose.get(-3);
    if (kty !== 2)
        throw new Error("cose_not_ec2");
    if (typeof crv !== "number")
        throw new Error("cose_bad_crv");
    if (!(x instanceof Uint8Array))
        throw new Error("cose_bad_x");
    if (!(y instanceof Uint8Array))
        throw new Error("cose_bad_y");
    return { credId, coseKey: { crv, x, y } };
}
export async function attestWithPasskey(stored, challengeBytes) {
