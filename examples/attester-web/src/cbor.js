function readU8(b, c) {
    const v = b[c.i];
    if (v === undefined)
        throw new Error("CBOR: out of bounds");
    c.i += 1;
    return v;
}
function readN(b, c, n) {
    const end = c.i + n;
    if (end > b.length)
        throw new Error("CBOR: out of bounds");
...
function decodeItem(b, c) {
    ...
    if (mt === 2) {
        ...
        if (typeof len === "bigint")
            throw new Error("CBOR: huge bstr");
    ...
    if (mt === 3) {
        const len = readUint(b, c, ai);
        if (typeof len === "bigint")
            throw new Error("CBOR: huge tstr");
    ...
    if (mt === 4) {
        const len = readUint(b, c, ai);
        if (typeof len === "bigint")
            throw new Error("CBOR: huge array");
        const arr = [];
        for (let i = 0; i < len; i++)
            arr.push(decodeItem(b, c));
        return arr;
    }
    // major type 5: map
    if (mt === 5) {
