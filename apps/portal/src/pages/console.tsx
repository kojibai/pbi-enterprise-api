    ...
    <div className="console">
      ...
      <div className="shell">
        ...
        <header className="topbar">
          ...
          <nav className="nav">
            <a className="navLink" href="/api-keys">API Keys</a>
            <a className="navLink" href="/billing">Billing</a>
            <a className="navLink" href="/terms">Terms</a>
            <a className="navLink" href="/privacy">Privacy</a>
          </nav>
        </header>
            ...
            <aside className="side">
              ...
              <div className="sideBody">
                ...
                <div className="planBtns">
                  <button className="btnGhost" disabled={!!busy || !priceIds.starter} onClick={() => checkout("starter")}>
                    Starter {PLAN_PRICE.starter}/mo
                  </button>
                  <button className="btnGhost" disabled={!!busy || !priceIds.pro} onClick={() => checkout("pro")}>
                    Pro {PLAN_PRICE.pro}/mo
                  </button>
                  <button className="btnPrimary" disabled={!!busy || !priceIds.enterprise} onClick={() => checkout("enterprise")}>
                    Enterprise {PLAN_PRICE.enterprise}/mo
                  </button>
                </div>

                <div className="hint">
                  Stripe checkout opens in the same tab. Plan limits apply automatically to all API keys.
                </div>
              </div>
            </aside>
          </div>
              ...
              {keys.length === 0 ? (
                ...
                keys.map((k) => (
                  <div className="trow" key={k.id}>
                    ...
                    <div className="cell">{k.is_active ? <span className="status ok">ACTIVE</span> : <span className="status off">REVOKED</span>}</div>
                    <div className="cell right">
                      {k.is_active ? (
                        <button className="btnDanger" disabled={!!busy} onClick={() => revokeKey(k.id)}>
...
.console{ min-height:100vh; color:var(--ink); font-family:var(--sans); }
...
.shell{
  max-width: 1120px;
  margin: 0 auto;
  padding: calc(16px + env(safe-area-inset-top)) calc(14px + env(safe-area-inset-right)) 72px calc(14px + env(safe-area-inset-left));
}
...
.h1{
  ...
  font-weight: 950; letter-spacing: -0.02em; line-height: 1.05;
  font-size: 34px;
}
.lead{ margin-top: 10px; color: rgba(255,255,255,.76); font-size: 14px; line-height: 1.65; max-width: 820px; }
.kpiRow{
  margin-top: 12px;
  display:grid;
